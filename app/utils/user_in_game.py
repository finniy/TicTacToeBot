...
def is_user_in_all_game(user_id: int) -> bool:
    """Проверяет, участвует ли пользователь в любой игре."""
...
def is_user_in_another_game(user_id: int) -> bool:
    """Проверяет, участвует ли пользователь в чужой игре (с двумя игроками)."""
    for game in active_games.values():
...
def is_user_in_his_game(user_id: int) -> bool:
    """Проверяет, участвует ли пользователь в своей игре (одинокая игра)."""
    for game in active_games.values():
